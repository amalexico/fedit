...
fedit -file $s -op insertafter -match "STRATEGIC NOTE" -textfile $r -v

$content = [IO.File]::ReadAllText($s)
if ($content -notmatch "afterblock T") {
    fedit -file $s -op insertafter -match "-beforeblock T   Destination: insert before named block (requires -lang)" -text "  -afterblock T    Destination: insert after named block (requires -lang)" -v
}

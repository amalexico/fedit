...
func mcpExecTool(name string, args map[string]any) mcpCallResult {
	...
	case "fedit_writeraw":
		return mcpDoWriteRaw(file, getStr("text"), start)
	case "fedit_insert":
	...
	case "fedit_insertafter":
		return mcpDoInsertMatch(file, getStr("match"), getInt("nth", 1), getStr("text"), false, getStr("block"), getStr("lang"), start)
	case "fedit_insertbefore":
		return mcpDoInsertMatch(file, getStr("match"), getInt("nth", 1), getStr("text"), true, getStr("block"), getStr("lang"), start)
	case "fedit_move":
...
func mcpDoWriteRaw(file, text string, start time.Time) mcpCallResult {
	content := strings.Split(text, "\n")
	if len(content) == 0 || (len(content) == 1 && content[0] == "") {
		return mcpErrorResult("Nothing to write")
	}
	if err := writeLines(file, content); err != nil {
		return mcpErrorResult(fmt.Sprintf("Error writing file: %v", err))
	}
	msg := fmt.Sprintf("Wrote %d line(s) to %s (raw, no escape expansion)", len(content), file)
	msg += mcpStats("writeraw", file, 0, len(content), "", start)
	return mcpOK(msg)
}

func mcpDoInsert(file string, afterLine int, text string, cleanfirst bool, start time.Time) mcpCallResult {
	lines, err := readLines(file)
	if err != nil {
		return mcpErrorResult(fmt.Sprintf("Error reading file: %v", err))
	}
...
func mcpDoInsertMatch(file, match string, nth int, text string, before bool, block, lang string, start time.Time) mcpCallResult {
	lines, err := readLines(file)
	if err != nil {
		return mcpErrorResult(fmt.Sprintf("Error reading file: %v", err))
	}
	...
	if block != "" {
		// Block-aware: resolve the function/class/resource range, then insert
		// before its opening line or after its closing line.
		blockStart, blockEnd, bErr := resolveBlock(lines, lang, block)
		if bErr != nil {
			return mcpErrorResult(fmt.Sprintf("block: %v", bErr))
		}
		if before {
			insertAfter = blockStart - 1
		} else {
			insertAfter = blockEnd
		}
	} else {
		if match == "" {
			return mcpErrorResult("requires match or block parameter")
		}
		hits := findMatches(lines, match)
		if len(hits) == 0 {
	...
	if insertAfter < 0 || insertAfter > len(lines) {
		return mcpErrorResult(fmt.Sprintf("Insert point %d out of range (file has %d lines)", insertAfter, len(lines)))
	}
	linesBefore := len(lines)
	result := make([]string, 0, len(lines)+len(newLines))
	...
	msg := fmt.Sprintf("Inserted %d line(s) %s %q (%d total now)", len(newLines), direction, targetDesc, len(result))
	msg += mcpStats("insert"+direction, file, linesBefore, len(result), targetDesc, start)
	return mcpOK(msg)
}
...
func mcpDoFields(file string, col int, delim string, x bool, start time.Time) mcpCallResult {
	if col < 1 {
